...
for e in range(setting.epochs):
    ...
    for i, (x, t, s, y, y_t, y_s, reset_h, active_users) in enumerate(dataloader):
        ...
        optimizer.zero_grad()
        loss, h = trainer.loss(x.detach(), t.detach(), s.detach(), y.detach(), y_t.detach(), y_s.detach(), h.detach(), active_users.detach())
	    # loss, h = trainer.loss(x.detach(),t.detach(),s.detach(),y.detach(),y_t.detach(),y_s.detach(),h.detach(),active_users.detach())
    ...
    if (e+1) % 1 == 0:
        ...
        f = open('./results/gowalla.txt', 'a')
        print(f'Epoch: {e+1}/{setting.epochs}')
        f.write(f'Epoch: {e+1}/{setting.epochs}\n')
        print(f'Used learning rate: {scheduler.get_lr()[0]}')
        f.write(f'Used learning rate: {scheduler.get_lr()[0]}\n')
        print(f'Avg Loss: {epoch_loss}')
        f.write(f'Avg Loss: {epoch_loss}\n')
        f.close()
        
    if (e+1) % setting.validate_epoch == 0:
        f = open('./results/gowalla.txt', 'a')        
        print(f'~~~ Test Set Evaluation (Epoch: {e+1}) ~~~')
        f.write(f'~~~ Test Set Evaluation (Epoch: {e+1}) ~~~\n')
        f.close()
